...
amass_test_split = []
amass_vald_split = ['BMLmovi']
amass_train_split = []
# amass_train_split = ['ACCAD', 'CMU']
...
def read_single_sequence(split_name, dataset_name, folder, seq_name, body_models, target_fps, max_fps_dist,
    ...
    for subject in tqdm(subjects):
        ...
        for action in actions:
            ...
            if args.selected_type == None or (args.selected_type in seq_raw_labels):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--selected_type', type=str, help='selected a particular type to be dataset',
                        default=None)
    ...
    for split_name in amass_splits.keys():
        ...
        if args.selected_type == None:
            db_file += '_{}_BMLmovi.pt'.format(split_name)
        else:
            db_file += '_{}_BMLmovi_{}.pt'.format(split_name, args.selected_type)
        if args.quick_run:
