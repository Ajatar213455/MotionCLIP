...
amass_test_split = []
amass_vald_split = []
# amass_train_split = ['ACCAD', 'HumanEva', 'CMU']
amass_train_split = ['ACCAD', 'HumanEva']
...
def read_data(folder, split_name,dataset_name, target_fps, max_fps_dist, joints_to_use, quick_run,babel_labels, clip_images_dir=None):
    ...
    print("clip_images_path = ", clip_images_path)
    ...
    for subject in tqdm(subjects):
        ...
        for action in actions:
            ...
            if babel_dict['frame_ann'] is None:
                if len(seq_raw_labels) != 1:
                    print(f"seq_raw_labels = {seq_raw_labels}, seq_proc_label = {seq_proc_label}")
                    continue
                    raise NotImplementedError
                babel_dict['frame_ann'] = {'labels': [{'start_t': 0, 
                                                      'end_t': babel_dict['dur'],
                                                      'raw_label': seq_raw_labels[0],
                                                      'proc_label': seq_proc_label[0]}]}
            ...
            for label_dict in babel_dict['frame_ann']['labels']:
                ...
                subMotion = data['poses'][start_frame:end_frame]
                raw_label, proc_label = label_dict['raw_label'], label_dict['proc_label']
                if raw_label == "transition" or proc_label == "transition": continue

                if not raw_label in labelDict_raw.keys():
                    labelDict_raw[raw_label] = len(labelDict_raw)
                if not proc_label in labelDict_proc.keys():
                    labelDict_proc[proc_label] = len(labelDict_proc)
                ...
                if target_fps is not None:
                    ...
                    pose = subMotion[0::sampling_freq, joints_to_use]
                    pose_all = subMotion[0::sampling_freq, :]

                else:
                    # don't sample
                    pose = subMotion[:, joints_to_use]
                    pose_all = subMotion[:, :]
                ...
                if quick_run:
                    ...
                    text_raw_labels.append(raw_label)
                    text_proc_labels.append(proc_label)
                    action_cat.append(frame_action_cat)

    print("labelDict_raw.keys = {}, len(raw) = {}".format(labelDict_raw.keys(), len(labelDict_raw)))
    print("labelDict_proc.keys = {}, len(proc) = {}".format(labelDict_proc.keys(), len(labelDict_proc)))
...
def get_babel_labels(babel_dir_path):
    ...
    for file in l_babel_dense_files:
        ...
        for seq_id, seq_dict in data.items():
            seperateSign = '/'
            # print(seq_dict['feat_p'].split(seperateSign)[1:])
            npz_path = os.path.join(*(seq_dict['feat_p'].split(seperateSign)[1:]))
            seq_dict['split'] = file
...
if __name__ == '__main__':
    ...
    for split_name in amass_splits.keys():
        ...
        db_file += '_{}_forACTOR.pt'.format(split_name)
        if args.quick_run:
